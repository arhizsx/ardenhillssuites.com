...
function eb_user_sing_up() {
    ...
    $user_nice_name = strtolower($_GET['eb_user_sign_up_email']);
...
if (!function_exists('eagle_booking_append_booking_button_menu') && eb_get_option('eb_header_button')):
    ...
    if ( $eb_menu_id != '') {
        ...
        if ( $eb_menu_id == eb_get_option('button_menu') || $eb_menu_tr_id == $eb_menu_tr_button ) {
            $items .= '<li class="menu_button"><a href="' . $eagle_booking_button_action . '" class="btn eb-btn" target="' . $eagle_booking_button_target . '"><i class="icon-calendar"></i>' . esc_html__('BOOK ONLINE', 'eagle-booking') . '</a></li>';
        }
    }
...
if (!function_exists('eb_get_js_settings')):
    function eb_get_js_settings() {
        ...
        $js_settings['eb_room_slider_loop'] = eb_get_option('room_slider_loop');
        ...
        $js_settings['eb_slide_up_search_form'] = eb_get_option('slide_up_search_form');
        $js_settings['eb_slide_up_search_form_style'] = eb_get_option('slide_up_search_form_style');
...
/* --------------------------------------------------------------------------s
 * Check if the page is an EB Page
 * Since  1.3.3.6
---------------------------------------------------------------------------*/

function is_eb_page( $except = '' ) {
    ...
    //  Current Page ID
    $eb_current_page_id = get_queried_object_id();
...
add_filter('body_class', 'eb_body_class', 99);


/* --------------------------------------------------------------------------
 * Append SlideUp search form
 * Since  1.3.3.6
---------------------------------------------------------------------------*/
if ( !function_exists('eb_slide_up_search_form_dom') ) {

    function eb_slide_up_search_form_dom() {

        if ( eb_get_option( 'slide_up_search_form' ) == true ) {

            echo '<div class="eb-popup-search-form-btn"> <span class="btn-text">Book Your Stay</span> <div class="btn"> <i class="icon-calendar"></i></div></div>';

            echo '<div class="eb-search-form eb-slide-search-form">';

            echo'<div class="eb-container">';

            include EB_PATH . '/core/admin/form-parameters.php';

            echo '<form id="search-form" class="eb-botton-form" action="' .$eagle_booking_action. '" method="get" target="' .esc_attr( $eagle_booking_target ). '">';

            echo '<div class="eb-form-fields">';
            ...
            */
            ...
            echo '<div class="eb-field-button">';
            echo '<button id="eb_search_form" class="btn eb-btn eb-btn-search-form" type="submit">';
            echo '<i class="icon-arrow-right"></i><span class="eb-btn-text">'.__('Check Availability', 'eagle-booking').'</span>';
            echo '</button>';
            echo '</div>';
...
if ( !function_exists('eb_breadcrumb') ) {
    ...
    function eb_breadcrumb() {
        ...
        echo '<li class="item"><a href="'.get_home_url().'">'.__('Home', 'eagle-booking').'</a></li>';
